...
//var rpPly = 99; // NB RP change
//function setPly(n) {
//	rpPly = n;
//	console.log("setPly", rpPly);
//}
...
function Search(finishMoveCallback, maxPly, finishPlyCallback) {
	...
	for (i = 1; i <= maxPly && g_searchValid; i++) {
		...
		if (!g_searchValid) break; // when time expires
...
var knightAdj = [-200, -100, -50, -50, -50, -50, -100, -200,
      -100, 0, 0, 0, 0, 0, 0, -100,
...
var bishopAdj = [-50, -50, -25, -10, -10, -25, -50, -50,
...
var rookAdj = [-60, -30, -10, 20, 20, -10, -30, -60,
...
var kingAdj = [50, 150, -25, -125, -125, -25, 150, 50,
...
self.onmessage = function (e) {
	...
	if (e.data.match("^position") == "position") {
		...
		if (parts.length > 1) g_timeout = parts[1];
		if (parts.length > 2) rp_ply = parts[2];
		if (parts.length > 2) rp_rand = parts[3];
		console.log("onmessage", e.data, g_timeout, rp_ply, rp_rand);
		//g_timeout = parseInt(e.data.substr(7, e.data.length - 7), 10);
		//console.log("search message ", g_timeout, rp_ply);
		Search(FinishMoveLocalTesting, rp_ply, FinishPlyCallback);
